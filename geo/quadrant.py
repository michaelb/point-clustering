...
class Quadrant:
    ...
    def lies(self, other, distance):
        distance2 = distance**2
        distancemax = (distance * (1 +2 *1.415)) ** 2
        for p1 in self.list_point_inside:
            for p2 in other.list_point_inside:
                if p1.distance_carre_to(p2) <= distance2:
                    return True
                elif p1.distance_carre_to(p2) > distancemax:
                    return False
        return False
